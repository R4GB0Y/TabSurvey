...
class TabTransformer(BaseModelTorch):
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        X = np.array(X, dtype=float)   # changed from np.float to float
        X_val = np.array(X_val, dtype=float) # changed from np.float to float
    ...
    def predict_helper(self, X):
        self.model.eval()
        X = np.array(X, dtype=float) #changed dtype from np.float to float
        X = torch.tensor(X).float()
